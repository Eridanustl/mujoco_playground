...
def main():
  ...
  for i, name in enumerate(env._mj_model.joint(joint_ids[i]).name
                            for i in range(len(joint_ids))):
    print(f"  [{i:2d}] {name:30s}  [{jnt_range_low[i]:+.4f}, {jnt_range_high[i]:+.4f}]")
  print()
  ...
  for i in range(len(joint_ids)):
    ...
    if violations_high[i]:
      flag = f" *** ABOVE LIMIT by {qpos_after_reset[i] - jnt_range_high[i]:.6f}"
    if flag:
      print(f"    [{i:2d}] qpos={qpos_after_reset[i]:+.6f}  "
            f"range=[{jnt_range_low[i]:+.6f}, {jnt_range_high[i]:+.6f}]{flag}")

  has_nan = np.any(np.isnan(qpos_after_reset)) or np.any(np.isnan(qvel_after_reset))
  ...
  for i in range(len(joint_ids)):
    ...
    print(f"    [{i:2d}] default={default_pose[i]:+.4f}  reset_qpos={qpos_after_reset[i]:+.4f}  "
          f"diff={diff:+.6f}  action_scale={action_scale[i]:.4f}  "
          f"range=[{jnt_range_low[i]:+.4f}, {jnt_range_high[i]:+.4f}]")
  print()
  ...
  for step_i in range(20):
    ...
    print(f"  term/joint_limit = {float(state.metrics['term/joint_limit']):.4f}")
    print(f"  NaN in qpos: {np.any(np.isnan(qpos_now))}, NaN in qvel: {np.any(np.isnan(qvel_now))}")
    print(f"  qpos below limit: {np.sum(violations_low)}, above limit: {np.sum(violations_high)}")
    ...
    for i in range(len(joint_ids)):
      ...
      if flag:
        print(f"    [{i:2d}] qpos={qpos_now[i]:+.6f}  "
              f"range=[{jnt_range_low[i]:+.6f}, {jnt_range_high[i]:+.6f}]{flag}")
