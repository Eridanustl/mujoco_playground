...
TARGET_POSITIONS = {
    "lf1": np.array([0.0836, -0.0347, 0.0393]),
...
# F1/F2 fingertip (L2) joints need a positive initial value so the fingers
# are not fully straight – this avoids a kinematic singularity where mink
# cannot distinguish L0 from L2 contributions.
# F0 (thumb) has no tip-position task, so it does not need seeding.
...
def main():
  ...
  def reset_sim():
    ...
    # Give L2 joints a small positive angle to avoid singularity.
