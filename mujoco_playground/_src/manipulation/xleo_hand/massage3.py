...
class Massage(mjx_env.MjxEnv):
  ...
  def _post_init(self) -> None:
    ...
    self._traj_qpos = jp.array(traj["qpos"])  # (T, 28)
    self._traj_qvel = jp.array(traj["qvel"])  # (T, 28)
  ...
  def _maybe_apply_perturbation(self, state: mjx_env.State) -> mjx_env.State:
    ...
    # Check if we are currently inside an active perturbation.
    ...
    # must not be in an active perturbation, and enough wait time has passed
    # since the last perturbation ended.
    time_since_pert_end = steps - (last_pert_step + info["pert_duration_steps"])
    start_pert = (~in_pert) & (time_since_pert_end >= info["pert_wait_steps"])
    start_pert &= steps > 0  # No perturbation at step 0.

    # Update last_pert_step and recompute duration/in_pert for the new perturbation.
